...
ffmpeg -err_detect ignore_err -listen 1 -i tcp://0.0.0.0:5000 -c:v libx265 -preset veryfast -x265-params "keyint=120:min-keyint=120" -c:a aac -ac 2 -f mp4 -movflags frag_keyframe+empty_moov - | ffmpeg -err_detect ignore_err -i pipe: \
...
#-c:v libx265 -preset veryfast -x265-params "keyint=120:min-keyint=120"
