...
ffmpeg -err_detect ignore_err -listen 1 -i tcp://0.0.0.0:5000 -map 0:v:0 -map 0:a:0 -map 0:s:0 -c:v libsvtav1 -g 60 -preset 11 -c:a aac -ac 2 -c:s mov_text -metadata:s:s:0 language=eng -f mp4 -movflags empty_moov+frag_keyframe - | ffmpeg -err_detect ignore_err -i pipe: \
