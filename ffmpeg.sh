...
ffmpeg -err_detect ignore_err -listen 1 -i tcp://0.0.0.0:5000 -c:v libx265 -preset faster -c:a aac -ac 2 -f mpegts - | ffmpeg -err_detect ignore_err -i pipe: \
-c copy -tag:v hvc1 -f hls -hls_time 2 -hls_list_size 1000 -hls_start_number_source epoch -hls_flags delete_segments -hls_segment_type mpegts /tmp/hls/tv.m3u8 \
...
# subtitles don't work by default with fmp4
#-f mp4 -movflags frag_keyframe+empty_moov
#-c:v libx264 -preset fast -crf 23 -x264-params keyint=60
#-c:v libsvtav1 -g 60 -preset 11
#-c:v libx265 -preset faster -x265-params keyint=60
