...
nc -l 5000 | ffmpeg -f lavfi -i "movie=pipe\\\:[out0+subcc],amovie=pipe\\\:[out1]" -map 0 -c copy -f mpegts - | ffmpeg -listen 1 -i tcp://0.0.0.0:5000 -c:v libx264 -preset fast -g 60 -keyint_min 60 -c:a aac -ac 2 -f mp4 -movflags empty_moov+frag_keyframe - | ffmpeg -i pipe: \
