...
| ffmpeg -err_detect ignore_err -i pipe: -c:v libx264 -x264-params "keyint=60:scenecut=0" -preset veryfast -crf 25 -c:a aac -ac 2 -c:s mov_text -r 30 -f mp4 -movflags frag_keyframe+empty_moov - \
| ffmpeg -err_detect ignore_err -i pipe: \
-c copy -f hls -hls_time 2 -hls_list_size 5400 -hls_flags delete_segments+append_list -hls_segment_type fmp4 /var/www/hls/tv.m3u8 \
...
#-c:v libx264 -preset fast -crf 25 -x264-params "keyint=60:scenecut=0"
