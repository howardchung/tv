...
ffmpeg -err_detect ignore_err -listen 1 -i tcp://0.0.0.0:5000 -c:v libsvtav1 -g 60 -preset 11 -c:a aac -ac 2 -f mp4 -movflags empty_moov+default_base_moof+frag_keyframe - | ffmpeg -err_detect ignore_err -i pipe: \
-c copy -c:s mov_text -f hls -hls_time 2 -hls_list_size 2000 -hls_start_number_source epoch -hls_flags delete_segments -hls_segment_type fmp4 /tmp/hls/tv.m3u8 \
-c copy -c:s mov_text -f dash -adaptation_sets "id=0,streams=v id=1,streams=a" -window_size 2000 -frag_duration 2 /tmp/dash/tv.mpd \
