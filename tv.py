...
def launch(id):
    ...
    stream = subprocess.Popen('dvbv5-zap --adapter=' + adapter + ' --input-format=ZAP -c channels.conf -o - "' + id + '" | ffmpeg -err_detect ignore_err -i pipe: -c:v libx264 -preset superfast -x264-params "keyint=60:scenecut=0" -c:a aac -ac 2 -r 30 -f flv rtmp://5.78.115.83:5000', shell=True, preexec_fn=os.setsid)
