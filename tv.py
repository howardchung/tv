...
def launch(id):
    ...
    #-f hls -hls_time 6 -hls_list_size 600 -hls_flags delete_segments -hls_segment_type fmp4 /mnt/watchparty-hls/tv.m3u8
    #-f flv rtmp://5.78.115.83:5000
    # Need to set env var since we're using old drivers (not iHD)
    os.environ["LIBVA_DRIVER_NAME"] = "i965"
    subprocess.run(['rm', '/mnt/watchparty-hls/*'])
    stream = subprocess.Popen('dvbv5-zap --adapter=' + adapter + ' --input-format=ZAP -c channels.conf -o - "' + id + '" | ffmpeg -i pipe: -c:v libx264 -preset superfast -x264-params "keyint=60:scenecut=0" -c:a aac -ac 2 -r 30 -f nut - | ffmpeg -i pipe: -c copy -f hls -hls_time 6 -hls_list_size 600 -hls_flags delete_segments -hls_segment_type fmp4 /mnt/watchparty-hls/tv.m3u8', shell=True, preexec_fn=os.setsid)
