...
def launch(id):
    ...
    #-c:v libx265 -I 60 -preset superfast
    #-c:v libsvtav1 -g 60 -preset 11
    #-c:v copy
    #-b:v 4M
    # Need to set env var since we're using old drivers (not iHD)
    os.environ["LIBVA_DRIVER_NAME"] = "i965"
    stream = subprocess.Popen('dvbv5-zap --adapter=' + adapter + ' --input-format=ZAP -c channels.conf -o - "' + id + '" | ffmpeg -err_detect ignore_err -i pipe: -c:v libx265 -I 60 -preset superfast -c:a aac -ac 2 -r 30 -f mpegts tcp://5.78.115.83:5000', shell=True, preexec_fn=os.setsid)
