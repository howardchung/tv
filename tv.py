...
def launch(id):
    ...
    #-vf scale=-1:720
    #-vf fps=30
    stream = subprocess.Popen('dvbv5-zap --adapter=' + adapter + ' --input-format=ZAP -c channels.conf -o - "' + id + '" | ffmpeg -err_detect ignore_err -i pipe: -tune zerolatency -b:v 7M -c:v libx264 -preset ' + preset + ' -x264-params keyint=60 -c:a aac -f flv rtmp://5.161.147.222/live/' + id, shell=True, preexec_fn=os.setsid)
