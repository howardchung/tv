...
def launch(id):
    ...
    stream = subprocess.Popen('dvbv5-zap --adapter=' + adapter + ' --input-format=ZAP -c channels.conf -o - "' + id + '" | ffmpeg -err_detect ignore_err -i pipe: -c:v libx264 -preset superfast -b:v 5M -c:a aac -ac 2 -r 30 -f mp4 -movflags frag_keyframe+empty_moov tcp://5.78.115.83:5000', shell=True, preexec_fn=os.setsid)
