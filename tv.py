...
def launch(id):
    ...
    stream = subprocess.Popen('dvbv5-zap --adapter=' + adapter + ' --input-format=ZAP -c channels.conf -o - "' + id + '" | ffmpeg -i pipe: -c:v libx264 -preset superfast -x264-params "keyint=60:scenecut=0" -b:v 3M -c:a aac -ac 2 -r 30 -f nut - | ffmpeg -i pipe: -c copy -tag:v avc1 -tag:a mp4a -f dash -seg_duration 4 -window_size 900  /mnt/watchparty-hls/tv.mpd', shell=True, preexec_fn=os.setsid)
