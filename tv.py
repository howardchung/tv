...
def launch(id):
    ...
    stream = subprocess.Popen('dvbv5-zap --adapter=' + adapter + ' --input-format=ZAP -c channels.conf -o - "' + id + '" | ffmpeg -err_detect ignore_err -i pipe: -c:v libx264 -preset veryfast -x264-params "keyint=60:scenecut=0" -b:v 3M -c:a aac -ac 2 -r 30 -f nut | ffmpeg -i pipe: -c copy -f hls -hls_time 2 -hls_list_size 1800 -hls_flags delete_segments -hls_segment_type fmp4 -hls_start_number_source epoch /backblaze/tv.m3u8', shell=True, preexec_fn=os.setsid)
