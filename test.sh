...
#dvbv5-zap --adapter=1 --input-format=ZAP -c channels.conf -o - "king-hd" | ffmpeg -i pipe: -c:v libx264 -preset superfast -c:a aac -ac 1 -f flv rtmp://5.161.147.222:1935/live/king-hd
...
#ffmpeg -i rtmp://localhost:1935/transcode/$name -vf scale=-1:480 -c:v copy -c:a copy -f flv rtmp://localhost:1935/live/$name
